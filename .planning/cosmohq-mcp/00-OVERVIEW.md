...
- 2026-09-12 19:30 BRT: "all prompts done" check. ADS-12 merged (`6801961`, api-main ff, API restarted 18:47, MCP rebuilt) and it carried ADS-10's launch code without ADS-10's tests → `ADS-10b`. ADS-11 not started. Pre-existing bug fixed directly: paid Reddit/LinkedIn rows were filtered out of `cosmohq_ad_campaigns` / `cosmohq_ad_test_report` (allowlist said `reddit_ads`) → `90bc7fd`, main pushed, MCP rebuilt. STL-01 landed (`18fdd09` outer) but with Markdown bold and a pt-BR privacy link in en-US → `STL-01b`. PAY-01/02, STL-02 untouched. XC: `feat/xcode-cloud-api-runner` (dced2b8, 8428e13) pushed with `scripts/xcloud.sh` working (status reads build 1) + E2E scheme + probe plan; all four XC files remain, local run 21:47Z left a 0-byte bundle, no new cloud build; local `main` still unpushed (c3109a1) and the checkout was left on the feature branch.

- 2026-09-12 21:15 BRT: second "all prompts done" check. STL-01b landed (`eed2f5d`). PAY-01/02 landed (`c382d29`, app main, pushed; Debug build green here; prompt files left, unit tests unverified from a Background session). Not touched: ADS-10b, ADS-11, STL-02, XC-02..05b (XC work still on `feat/xcode-cloud-api-runner`, merges clean into main, 0 conflicts).
...
- 2026-09-15: **NUM-03b — NUM-03 finished.** Merged to `main` (`abfd91f`, merge of `83fd971`; conflicts only in `client.go`/`CLAUDE.md`, both additive). `tools/list` over stdio prints **50 tools** incl. `cosmohq_connect`. `api-main` fast-forwarded, `:3003` restarted from `.../api-main/api` and `~/.local/bin/cosmohq-mcp` rebuilt (owner OK). Live gates with the owner: `cosmohq_connect platform=google` → `status: connected` (account CosmoHQ, ENABLED) and `cosmohq_sync_insights` synced 4 campaigns with `errors: []`, clearing the stale `invalid_grant` `syncError`; Reddit already `connected` (`status` returns the ad account); RevenueCat reads live (`cosmohq_sales overview` → MRR 1, no `revenuecat_not_connected`). `cosmohq_app_numbers appId=cmn9cnfhx000rwb79k5sxx17p windowDays=30` now returns `blocked: []`.
  - **Open (follow-up):** `cosmohq_connect platform=revenuecat action=status` still reports `disconnected` because it reads `/v1/v1-wizard/apps/{id}/project`, which exposes no RevenueCat fields, while `internal/iap` reads the creds from `COALESCE(ac, pub)`. The manual `/keys` flow therefore can't "see" the existing creds; point the status check at the same source the IAP sync uses.
